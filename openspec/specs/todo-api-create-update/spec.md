...
The API SHALL provide a `POST /todos` endpoint that accepts a JSON body with a todo name and creates a new todo document in MongoDB.
...
The API SHALL provide a `PATCH /todos/:id` endpoint that accepts a JSON body with fields to update on an existing todo document.
